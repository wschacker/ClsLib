...
- (NSString *)convertToServerInterval
{
    NSTimeInterval interval = [self timeIntervalSince1970];
    long long serverTime = (long long)interval * 1000;
    NSString * str = [NSString stringWithFormat:@"%lld", serverTime];
    return str;
}

+ (NSDate *)dateWithServerInterval:(NSString *)serverTime
{
    double serverTimell = [serverTime doubleValue];
    NSDate * date = [NSDate dateWithTimeIntervalSince1970:(serverTimell / 1000.0f)];
    return date;
}

+ (NSDate *)dateWithServerIntervalInNumber:(NSNumber *)serverTimeNum
{
    NSString * str = [NSString stringWithFormat:@"%lld", (long long)([serverTimeNum doubleValue])];
    return [NSDate dateWithServerInterval:str];
}
